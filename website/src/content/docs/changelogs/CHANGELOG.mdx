...
- Added velocity layers to sample variants. The new `VELOCITY` variant `MODE` plays the slot whose `VMIN`-`VMAX`
  range covers the note's velocity; slots whose ranges overlap alternate, and velocities in a gap play slot 1.
  - Importing a folder sets this up when every sample on a note has a `vel` tag in its filename, such as
    `Snare_vel40.wav`.

#### <ins>Kits</ins>

- Added numbered choke groups. A kit drum set to `Choke` polyphony can be put in one of 8 groups in the new
  `VOICE > CHOKE GROUP` menu, and then only chokes drums in the same group.
  - Existing kits load with every `Choke` drum in group 1, so they behave as before.
  - Can be turned off in `SETTINGS > COMMUNITY FEATURES > CHOKE GROUPS (CGRP)`; group numbers already saved in a
    song are kept either way.
  - See [Numbered Choke Groups](/features/community_features#338---numbered-choke-groups) for details.
...
- Added `EXPORT CHOKE GROUPS` to the kit drum export's `CONFIGURE EXPORT` menu. Drums that share a choke group are
  exported together into one file, since they have to sound together for the choke to happen; every other drum is
  still exported on its own. See [Export Choke Groups](/features/audio_export#export-choke-groups).
