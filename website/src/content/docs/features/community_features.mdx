...
- Each oscillator sample zone in Synth and Kit sound sources can now hold up to 4 sample variants: the primary
  sample plus up to 3 alternates, played in rotation (or picked by note velocity) each time the zone is
  triggered - the classic multi-sample-per-pad workflow known from MPC-style samplers, useful for removing the
  "machine gun" effect on repeated drum hits.
