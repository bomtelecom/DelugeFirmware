...
In `KIT INSTRUMENT CLIP VIEW`, if the [Numbered Choke Groups](/features/community_features#338---numbered-choke-groups) community feature is on and the kit's drums span more than one choke group, you can instead export `CHOKE GROUP's` - one file per numbered choke group, mixing together every drum that shares a group - by enabling `EXPORT CHOKE GROUPS` (see below).
...
- Note 6: When exporting Choke Groups, a group is only exported if it contains at least one eligible (unmuted, non-empty) Drum
...
- In terms of the length of each rendered audio file:
  ...
  - When exporting Choke Groups, all drums in a group are played together until the end of the longest drum row in that group is reached.
...
- Exporting one file per numbered choke group instead of one file per drum is disabled by default
- The option to enable it is only shown at all when the current kit's drums actually span more than one distinct choke group - if every drum shares a single group, choke group export would just reproduce the same single file a normal kit export produces, so there's nothing to choose
